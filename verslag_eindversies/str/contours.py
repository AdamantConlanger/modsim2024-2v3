        ...
        if plot_periods:
            ...
            if len(nonzero_periods) == 0:
                ...
                if np.diff(extent[:2]) <= sliver:
                    ...
                    x_step = dx
                    while 16 * x_step < np.diff(extent[:2]):
                        x_step = 2 * x_step
                ...
                if np.diff(extent[2:]) <= 10**-6:
                    ...
                    y_step = dy
                    while 16 * y_step < np.diff(extent[2:]):
                        y_step = 2 * y_step
                ...
                contours = axs.contour(periods_data.T, contour_fills.levels, origin='lower', colors='k', extent=shifted_extent)
