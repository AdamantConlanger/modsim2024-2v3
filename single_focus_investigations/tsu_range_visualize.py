...
def make_label(x0, y0, k1, k2, k3, k4, p, q, is_focus):
    result = ""
    if is_focus[0]:
        if result != "":
            result += "; "
...
def visualize(f, initials_list, coefficients_list, interval_list, evaluations_list, solution_list, is_focus, extrema_variables, *, suppress_legend=False, suppress_ghosts=False, time_restricted=None, broader_colors=False, colors_2d=False):
    ...
    for n in range(len(solution_list)):
        ...
        x, y = solution_out.y
        x0, y0 = initials_out
        k1, k2, k3, k4, p, q = coefficients_out
        if n == 1:
            the_subtitle = make_subtitle(x0, y0, k1, k2, k3, k4, p, q, is_focus)
        the_label = make_label(x0, y0, k1, k2, k3, k4, p, q, is_focus)
        if broader_colors:
            the_color = decide_color_broader([x0, y0, k1, k2, k3, k4, p, q], is_focus, extrema_variables)
        elif colors_2d:
            the_color = decide_color_2d([x0, y0, k1, k2, k3, k4, p, q], is_focus, extrema_variables)
        else:
            the_color = decide_color([x0, y0, k1, k2, k3, k4, p, q], is_focus, extrema_variables)
    ...
    the_title = "A graph of the truncated simple unreduced model."
