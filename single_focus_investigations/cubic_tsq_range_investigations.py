...
from cubic_tsq_range_visualize import visualize
...
def f(t, u, coeffs):
    ...
    mu1, mu2 = coeffs
    return np.array([
        x * x * x * y - x - math.sqrt(mu2 / mu1) * x + math.sqrt(mu1 * mu2),
        x - x * x * x * y
    ])


base_initials = [1.5, 0.5]  # list of starting values of the variables; first part of the parameters.
base_coefficients = [0, 3/50]  # list of coefficients for reaction speeds; second part of the parameters.
...
variations_coefficients = [np.linspace(1, 1.2, 51), None]
...
result = simulate(f, initials_list, coefficients_list, interval_list, evaluations_list, atol=10**-6, rtol=10**-5)
